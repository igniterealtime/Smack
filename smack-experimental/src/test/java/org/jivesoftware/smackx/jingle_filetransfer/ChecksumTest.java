...
public class ChecksumTest extends SmackTestSuite {
    ...
    public void parserTest() throws Exception {
        ...
        assertXmlSimilar(xml, checksum.toXML().toString());
        XmlPullParser parser = TestUtils.getParser(xml);
        ChecksumElement parsed = new ChecksumProvider().parse(parser);
        ...
        xml = "<checksum xmlns='urn:xmpp:jingle:apps:file-transfer:5' creator='initiator' name='name'>" +
                ...
                "</checksum>";
        assertXmlSimilar(xml, checksum.toXML().toString());
        parser = TestUtils.getParser(xml);
        parsed = new ChecksumProvider().parse(parser);
        assertXmlSimilar(xml, parsed.toXML().toString());
