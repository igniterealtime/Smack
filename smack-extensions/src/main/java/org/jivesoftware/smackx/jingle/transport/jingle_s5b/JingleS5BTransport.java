...
public class JingleS5BTransport extends JingleTransport<JingleS5BTransportElement> {
    ...
    private void establishBytestreamSession(XMPPConnection connection)
        ...
        if (ourSelectedCandidate == CANDIDATE_FAILURE) {
            connection.createStanzaCollectorAndSend(JingleS5BTransportManager.createCandidateError(this, connection));
            callback.onTransportFailed(new S5BTransportException.CandidateError(null));
            return;
        }
        ...
        connection.createStanzaCollectorAndSend(JingleS5BTransportManager.createCandidateUsed(this, connection, ourSelectedCandidate));
        connectIfReady();
    }
    ...
    private void connectIfReady() {
        ...
        if (nominated == theirSelectedCandidate) {
            ...
            XMPPConnection connection = session.getJingleManager().getConnection();
            ...
            try {
                ...
                Async.go(new Runnable() {
                    ...
                    public void run() {
                        try {
                            connection.createStanzaCollectorAndSend(JingleS5BTransportManager.createProxyError(JingleS5BTransport.this, connection));
            ...
            if (isProxy) {
                ...
                JingleElement candidateActivate = JingleS5BTransportManager.createCandidateActivated((JingleS5BTransport) nominated.getParent(), connection, nominated);

                try {
                    connection.createStanzaCollectorAndSend(candidateActivate)
                            .nextResultOrThrow();
    ...
    public IQ handleTransportInfo(JingleContentTransportInfoElement info, JingleElement wrapping) {
        switch (info.getElementName()) {
            ...
            case JingleS5BTransportInfoElement.CandidateUsed.ELEMENT:
                handleCandidateUsed(((JingleS5BTransportInfoElement.CandidateUsed) info).getCandidateId(), wrapping);
                break;
    ...
    private void handleCandidateUsed(String candidateId, JingleElement wrapping) {
        ...
        if (theirSelectedCandidate == null) {
            LOGGER.log(Level.SEVERE, "Unknown candidateID: " + candidateId);
    ...
    private void handleCandidateActivate() {
    ...
    private void handleCandidateError() {
     ...
     */
    private static final JingleS5BTransportCandidate CANDIDATE_FAILURE = new JingleS5BTransportCandidate(null, null, -1, null);
}
