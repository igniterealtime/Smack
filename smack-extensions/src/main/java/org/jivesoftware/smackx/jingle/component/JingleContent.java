...
public class JingleContent implements JingleTransportCallback, JingleSecurityCallback {
     ...
     * @return result.
     */
    private static IQ handleContentModify(JingleElement request) {
     ...
     * @return result.
     */
    private static IQ handleDescriptionInfo(JingleElement request) {
        return IQ.createErrorResponse(request, StanzaError.Condition.feature_not_implemented);
    }
     ...
     * @return result.
     */
    private static IQ handleSecurityInfo(JingleElement request) {
        return IQ.createErrorResponse(request, StanzaError.Condition.feature_not_implemented);
    }
     ...
     * @return result.
     */
    private static IQ handleSessionInfo(JingleElement request) {
        return IQ.createResultIQ(request);
    }
    ...
    public void onTransportFailed(Exception e) {
        ...
        if (getParent().isInitiator()) {
            try {
                ...
                LOGGER.log(Level.SEVERE, "Could not send transport-replace.", e1);
                handleContentTerminate(JingleReasonElement.Reason.failed_transport);
            }
        } else {
            handleContentTerminate(JingleReasonElement.Reason.failed_transport);
        }
