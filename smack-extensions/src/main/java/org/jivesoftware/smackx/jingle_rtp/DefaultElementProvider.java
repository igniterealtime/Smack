...
/**
 * A provider that parses incoming stanza NameElement into instances of the {@link Class} that it has
 ...
 */
public class DefaultElementProvider<EE extends AbstractElement> extends Provider<EE> {
    /**
    ...
    private static final Logger LOGGER = Logger.getLogger(DefaultElementProvider.class.getName());
     ...
     * @param elementName stanzas builder with the modified nameSpace
     */
    public DefaultElementProvider(Class<EE> c, String elementName) {
        stanzaClass = c;
        mElementName = elementName;
    }

    public DefaultElementProvider(Class<EE> c) {
        stanzaClass = c;
        mElementName = null;
    }
    ...
    public EE parse(XmlPullParser parser, int depth, XmlEnvironment xmlEnvironment, JxmppContext jxmppContext)
        ...
        AbstractElement.Builder<?> mBuilder = stanzaExtension.getBuilder(mElementName);
            ...
            case START_ELEMENT:
               ...
               Provider<?> provider = JingleContentProviderManager.getJingleContentElementProvider(name);
                ...
                if (provider == null) { //  && !JingleFileTransfer.NAMESPACE_V5.equals(namespace)) {
                    LOGGER.log(Level.WARNING, "No DefaultElement provider for EE<" + name + " " + namespace + "/>");
                }
                else {
                    // LOGGER.log(Level.SEVERE, "Provider for DefaultElement Element: " + name + " " + provider);
                    ...
                    if (childExtension != null) {
                        ...
                        LOGGER.log(Level.INFO, "Invalid Abstract Element: " + name);
